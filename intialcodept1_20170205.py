...
#set this one up with computer beforehand in terms of actually using the product
lowNote = getLowNote()

#this should be determined by button presses and within the 'while True:' bit in the final product, but it's outside now because it'd be annoying to enter a number every time a note needs to be played for this
chord = getChord(lowNote)

#this only exists because I have not yet created the stuff to deal with hardware yet
v = raw_input("velocity in m/s: ")

#main bit of main code
while True:
    #v = getV() -- this will get velocity from the accelerometer (after doing some calculus obvs)
    if v > 0 and n < 4:
        #chord = getChord(lowNote)
